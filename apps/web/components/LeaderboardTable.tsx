      ...
      <table className="w-full">
        <thead className="bg-[#120b22] font-pixel text-[9px] uppercase tracking-wider text-muted">
          <tr>
            ...
            {!compact && <th className="px-4 py-4 text-right">Msgs</th>}
            {!compact && <th className="px-4 py-4 text-right">Sealed</th>}
          </tr>
        </thead>
        <tbody className="tnum text-[19px]">
          {rows.map((r, i) => {
            const t = r.submission.totals;
            const medal = MEDAL[i];
            return (
              <tr key={r.login} className="border-t-4 border-[#120b22] hover:bg-[#22163d]">
                <td className="px-4 py-3 font-pixel text-[11px]" style={{ color: medal ?? '#a596c9' }}>
                  {medal ? '★' : ''}{String(r.rank).padStart(2, '0')}
                </td>
                ...
                <td className="px-4 py-3 text-right text-gold">{humanTokens(t.totalTokens)}</td>
                <td className="px-4 py-3 text-right">{money(t.cost)}</td>
                {!compact && <td className="px-4 py-3 text-right">{t.activeDays}</td>}
                {!compact && <td className="px-4 py-3 text-right">{humanTokens(t.messageCount)}</td>}
                {!compact && <td className="px-4 py-3 text-right text-muted">{relDate(r.submission.generatedAt)}</td>}
              </tr>
            );
          })}
        </tbody>
